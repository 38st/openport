...
describe("coverage labels", () => {
  it("shows exact counts and warns for either quotes or OI below 90%", () => {
    const coverage = { options: 100, quoted: 100, priced: 90, open_interest: 97 }
    expect(expiryCoverage(coverage)).toEqual({ label: "quoted 100/100 · priced 90/100 · OI 97/100", low: false })
    expect(expiryCoverage({ ...coverage, priced: 89 })?.low).toBe(false)
    expect(expiryCoverage({ ...coverage, quoted: 89 })?.low).toBe(true)
    expect(expiryCoverage({ options: 366, quoted: 366, priced: 247, open_interest: 366 })?.low).toBe(false)
    ...
    expect(expiryCoverage({ ...coverage, options: 10000, priced: 9999, open_interest: 9999 })?.label)
      .toBe("quoted 100/10000 · priced 9999/10000 · OI 9999/10000")
  })
